...
func main() {
	...
	bcEncVotes := zk.StringToVotesSolidity(*encBCVote)
	...
	witnessFilePath := fmt.Sprintf("%s/plonk_pub_input.pub", path)
	witnessFile, _ := os.Create(witnessFilePath)
	...
	fmt.Printf(hex.EncodeToString(buf.Bytes()))

	pubArray := pub.Bytes()
	fmt.Printf(hex.EncodeToString(pubArray[:]))
