...
func GetVKPlonk() plonk.VerifyingKey {
	byteArray, _ := os.ReadFile(filepath.Join(circuitSetupPath, "vk.bin"))
	vk := plonk.NewVerifyingKey(ecc.BN254)
	vk.ReadFrom(bytes.NewReader(byteArray))

	return vk
}

func GetCCSPlonk() constraint.ConstraintSystem {
	byteArray, _ := os.ReadFile(filepath.Join(circuitSetupPath, "ccs.bin"))
...
func GetPKPlonk() plonk.ProvingKey {
	byteArray, _ := os.ReadFile(filepath.Join(circuitSetupPath, "pk.bin"))
